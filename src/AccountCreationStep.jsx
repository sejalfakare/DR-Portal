...
const AccountCreationStep = ({ step, nextStep, handleChange, formData, progressWidth }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // You can add validation specific to the current step here if needed
        nextStep();
    };
            ...
            return (
                <>
                    ...
                    <fieldset className="form-section">
                        ...
                        <div className="form-grid">
                            <div className="form-group">
                                ...
                                <select id="title" className="form-select" required value={formData.title} onChange={handleChange}>
                                    <option value="Dr.">Dr.</option>
                                    <option value="Prof.">Prof.</option>
                                </select>
                            </div>
                            <div className="form-group">
                                <label htmlFor="regNumber">NMC / State Registration No.</label>
                                <input type="text" id="regNumber" className="form-input" required value={formData.regNumber} onChange={handleChange} />
                            </div>
                            <div className="form-group">
                                <label htmlFor="council">Registering Council</label>
                                <input type="text" id="council" className="form-input" placeholder="e.g., Maharashtra Medical Council" required value={formData.council} onChange={handleChange}/>
                            </div>
                            <div className="form-group">
                                <label htmlFor="regYear">Year of Registration</label>
                                <input type="number" id="regYear" className="form-input" placeholder="YYYY" min="1950" max="2025" required value={formData.regYear} onChange={handleChange}/>
                            </div>
                            <div className="form-group full-width">
                                <label htmlFor="specializations">Specialization(s)</label>
                                <input type="text" id="specializations" className="form-input" placeholder="e.g., Cardiology, Dermatology (comma-separated)" required value={formData.specializations} onChange={handleChange}/>
                            </div>
                            <div className="form-group full-width">
                                <label htmlFor="qualifications">Qualifications / Degrees</label>
                                <input type="text" id="qualifications" className="form-input" placeholder="e.g., MBBS, MD, DM (comma-separated)" required value={formData.qualifications} onChange={handleChange}/>
                            </div>
                        </div>
                    </fieldset>
                    ...
                    <fieldset className="form-section">
                        ...
                        <div className="form-grid">
                            <div className="form-group">
                                <label className="upload-label" htmlFor="medLicense">
                                    <span className="upload-icon">📄</span> Upload Medical License
                                    <span className="upload-text">{formData.medLicense ? formData.medLicense.name : 'Click to select a file'}</span>
                                    <input type="file" id="medLicense" className="file-input" required onChange={handleChange} />
                                </label>
                            </div>
                            <div className="form-group">
                                <label className="upload-label" htmlFor="photoId">
                                    <span className="upload-icon">🆔</span> Upload Government Photo ID
                                    <span className="upload-text">{formData.photoId ? formData.photoId.name : 'Click to select a file'}</span>
                                    <input type="file" id="photoId" className="file-input" required onChange={handleChange} />
                                </label>
                            </div>
                        </div>
                    </fieldset>
                    ...
                    <div className="form-group full-width profile-picture-group">
                        ...
                        <input type="file" id="profilePicture" className="form-input" onChange={handleChange} />
                    </div>
